 ...
 */
 public class Person {
	...
	public String getNif() 
		throws PropertyNotInitializedException, InvalidValueException, ValidadorNIF {
		...
		//Validar que sean 9 caraceteres
		if (nif.length() != 9) throw new ValidadorNIF("\"nif\" must have 9 characters");
		//Validar que ultima letra sea un caracter
		if (!Character.isLetter(nif.charAt(8))) throw new ValidadorNIF("Last \"nif\" character must be a letter");
		//Calculo de letra
		final String LETRAS_NIF = "TRWAGMYFPDXBNJZSQVHLCKE";
		int numerosNIF = Integer.parseInt(nif.substring(0, 8));
		char letraCalculada = LETRAS_NIF.charAt(numerosNIF % 23);
		if (nif.charAt(8) != letraCalculada) throw new ValidadorNIF("\"nif\" letter is incorrect");
		return nif;
	}
	/** setNif()
